...
void Game_Over()
{
    ...
    delay(x);
    while(1) ///masuh blm pass //baru (thee)
    {
    	GetCursorPos(&cpos);
...
void Gerak_peluru(TANK *tank,TANK *tank2,TANK *tank3,TANK *tank4,boolean *garuda)
{
    /*Kanan*/
    if(tank->ktemp==0)
        {
...
void CekHancur(TANK *tank)
{
    if(tank->Hancur)
    {
        SetObjek0(tank->posisi.kolom[0],tank->posisi.baris[0]);
        if(tank->Tank==1)
        {
            Set_player(&(*tank));
        }
